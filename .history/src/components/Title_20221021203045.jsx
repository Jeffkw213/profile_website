...
export default function Title({ Name, Description }) {
  var vw = 100; //getting the width of the window
  var vh = 200; //getting the height of the window
  function Stars() {
    const arr = [];    
    for (let i = 0; i < 100; i++) {
      arr.push(<div className={`bg-white w-2 h-2 rounded-full absolute top-[${vh}px] left-[${vw}px] rounded-full`} key={`${i}`}></div>);
    }
    return arr;
  }
  return (
    <div className="bg-black max-w-1/2 w-full px-4 absolute" id="home">
      <div>
        <Stars />
      </div>
